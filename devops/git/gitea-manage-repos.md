...
> * Create a new git repository `story_official` under `max` user.
...
> * Copy all files from location `/usr/finance` to the repository and commit/push your changes to the master branch. The commit message must be `"add stories"` (must be done in single commit).
> * Create a new branch `max_demo` from master.
